...
2026-09-21 に本家へログインして実測（詳細は 13.3）:

- 「最近の更新」は**初期10件＋スクロールで追加読み込み**。絞り込みは**あり**
- 「自分の課題」の既定の並び順は**期限日の昇順**、10件

未確認:

- `TODO(要確認)`: 「自分のプルリクエスト」の抽出条件（担当のみか、登録も含むか）と件数。
  トライアルスペースにGitリポジトリが無く実測できない
...
### 13. 本家にログインして実測した挙動（2026-09-21）

公開ドキュメントに記載が無く、画面を動かさないと分からないものを実測した。
対象は本家のトライアルスペース（サンプルプロジェクト `PRJ` / `PRJ_1`、課題24件）。

#### 13.1 課題一覧の既定値

`/find/<projectKey>` をパラメータ無しで開くと、本家自身が既定値付きのURLへ
書き換える。**これが既定値の一次情報**。

```
sort=UPDATED&order=false&limit=20&statusId=1&statusId=2&statusId=3
&simpleSearch=true&allOver=false&offset=0
```

| 項目 | 既定値 |
|---|---|
| 並び順 | **更新日の降順**（`sort=UPDATED` / `order=false`、見出しに `is_sort-down`） |
| 件数 | **20件** |
| 状態の絞り込み | **`statusId=1,2,3`（＝完了以外）**。完了は既定で出ない |
| 範囲 | `allOver=false`（プロジェクトをまたがない） |

#### 13.2 キーワード検索が見る範囲

**画面のパラメータ名は `keyword` ではなく `query`。**（APIは `keyword`。名前が違う）
`keyword=` を付けても無視され、全件が返る。

「最終納品」は `PRJ_1-11` の**詳細にのみ**出る語（件名には無い）。
`query=最終納品` で検索すると **`PRJ_1-11` の1件だけ**に絞れた。

→ **検索範囲は件名だけでなく詳細（description）を含む。**

- `TODO(要確認)`: コメント本文まで見るか。サンプルデータにコメントが無く、
  確かめるには本家へ書き込む必要があるため保留

#### 13.3 ダッシュボード

| ブロック | 実測 |
|---|---|
| 自分の課題 | **期限日の昇順**（9/14×4 → 9/15×4 → 9/16×2）。**10件**表示 |
| 自分の課題の絞り込み | 対象「担当(24) / 登録(24)」、期限日「すべて / 4日以内(0) / 今日まで(0) / 期限切れ(24)」 |
| 最近の更新 | **初期10件、スクロールで追加読み込み（無限スクロール）**。このスペースの全活動25件まで読み込まれ、そこで止まった |
| 最近の更新の絞り込み | **あり**（「フィルタ: すべて」と「表示設定」） |
| 最近の更新に出る活動 | 課題を追加 / 課題を更新 / **ドキュメントを追加** |

**注意**: 初回描画だけを見ると「10件で打ち止め」と誤読する。
スクロールで増えるため、**件数の上限ではなく初期表示件数**である。

#### 13.4 ガントチャート

URLは `?span=6&scale=<値>&startDate=YYYY/MM/DD&withParentRelation=true`。

- `scale` の値は **`days` / `weeks` / `months` / `quarters`**（既定は `days`）。
  単数形 `quarter` は無効で `days` にフォールバックする
- **四半期スケールは暦の四半期にスナップする**（表示開始日基準ではない）

| 指定した開始日 | 軸の先頭 | 判定 |
|---|---|---|
| 2026/09/14（第3四半期） | **7月** | 暦のQ3の先頭に戻る |
| 2026/05/10（第2四半期） | **4月** | 暦のQ2の先頭に戻る |

上段の帯は**暦年**（2027年が1月から始まる）。下段の目盛は月。

- **範囲内に該当する課題が1件も無いと、軸ごと描画されない**。
  「該当する課題が見つかりませんでした」とヒントだけが出る

#### 13.5 変更履歴の表示

課題画面の履歴は、コメント一覧に混ざって時系列で出る（`comment-item`）。
状態変更は **「状態: <変更前><変更後>」** の形で記録される（ラベルは「状態」）。

- `TODO(要確認)`: 記録されるフィールドの網羅。サンプルデータに状態変更しか
  無く、網羅を確かめるには本家へ書き込む必要があるため保留

#### 13.6 実測で見つかった実装との差分

| 項目 | 本家 | 実装 | 判定 |
|---|---|---|---|
| 課題一覧の既定ソート | 更新日の降順・20件 | 同じ（D11 / D12） | 一致 |
| キーワードが見る範囲 | 件名＋詳細 | 件名＋詳細（`buildIssueWhere`） | 一致 |
| ダッシュボード「自分の課題」 | 期限日の昇順・10件 | 同じ（`MY_ISSUES_LIMIT = 10`、`dueDate asc`） | 一致 |
| **課題一覧の既定の状態絞り込み** | **完了以外**（URLに `statusId=1,2,3` を明示して書き込む） | **既定なし。完了も混ざって出る** | **要判断（下記）** |
| ダッシュボード「最近の更新」 | 初期10件＋無限スクロール | 20件を一度に出す | 要判断（下記） |

**課題一覧の既定の状態絞り込みについて**

本家は `/find/<key>` をパラメータ無しで開くと、`statusId=1&statusId=2&statusId=3`
を**URLに書き込んでから**表示する。つまり「未指定なら完了以外」という暗黙の既定ではなく、
**既定値を明示したURLへ誘導する**作りになっている。

こちらは `statusId` の既定が無く、完了済みの課題が最初から一覧に混ざる。
この既定について文書化された決定は無く、単に考慮から漏れていた。

実装の選択肢:

1. 本家と同じく、`statusId` 未指定なら既定値付きURLへ誘導する（見た目も挙動も本家と同じ）
2. URLは変えず、未指定のときだけ内部で「完了以外」を既定にする（リダイレクトが増えない）
3. 現状のまま（すべて表示）を意図的な相違として残す

**注意**: この `statusId=1,2,3` は**画面の既定**であって、REST API `GET /issues` の既定ではない。
APIに同じ既定を入れるとAPI互換が壊れるので、直すなら画面側だけにする。

- `TODO(要確認)`: 追加した状態（id 5以降）を持つプロジェクトで、本家の既定が
  `1,2,3` のままか、追加状態も含めた「完了以外」になるか。
  検証には本家へ状態を追加する必要があるため保留

#### 13.7 標準4状態の色（12.1 の再確認）

`.status--1`〜`.status--4` の計算済みスタイルを再度読み、**12.1 の記載と完全に一致**した。
クラス名が `status--<id>` である点から、**id 1/2/3/4 と 未対応/処理中/処理済み/完了 の
対応も裏付けられた**。`displayOrder` の実値は画面に出ないため未確認のまま。

### 14. プロジェクトホーム（2026-09-23 に本家の画面で確認）

`/projects/<key>` に出る画面。**スペース全体のダッシュボード（11.1）とは別物**で、
1プロジェクトの中だけを見る。こちらには画面自体が無く、サイドバーの「ホーム」が
課題一覧を指していた。

見出しは「プロジェクトホーム：最近の更新」。RSSのアイコンが並ぶ。

#### 14.1 主部（左）— 最近の更新

そのプロジェクトの活動を時系列で出す。ダッシュボードの「最近の更新」と違い、
**1プロジェクトに限定**される。

- 日付の見出しでまとめる（`2026年9月13日(日)`）
- 1行は「アバター ＋ <名前>さんが <種別バッジ> を追加/更新 ＋ 相対時刻（10日前）」
- 課題の行は、キー・件名に続けて**本文の抜粋**を出し、長ければ「… もっと読む」
- 状態が変わった行は `[ 状態: 処理中 ]` のように変化を出す
- 各行に**コメントの追加**とスター（`0`）が付く
- 上部に「フィルタ : すべて」と「表示設定」
...
| ブロック | 中身 |
|---|---|
| 状態 | 横積みの棒グラフと「0% 完了」。下に 未対応 11 / 処理中 1 / 処理済み 0 / 完了 0 を状態の色付きバッジで並べる |
| バーンダウンチャート | 進行中のマイルストーンが無ければ「現在進行中のマイルストーンはありません。」と出し、ヘルプへのリンクを添える |
| マイルストーン | マイルストーンごとに進捗バーと「N% 完了」。無ければ「未設定」。カレンダーと編集のアイコンが付く |
| カテゴリー | カテゴリーごとに進捗バーと「N% 完了」。無ければ「未設定」。編集のアイコンが付く |

状態の色は 12.1 / 13.6 で確定したものと同じ。

- `TODO(要確認)`: 「表示設定」で何を切り替えられるか
- `TODO(要確認)`: 活動フィードの初期表示件数（ダッシュボードは初期10件＋無限スクロール。13.3）
...
| D11 | 課題一覧の既定ソート | `updated` の降順 | 本家の既定は未記載。`order` の既定が `desc` であること、「最近動いた課題から見る」のが実務の基本であることから。インデックス `(project_id, updated_at desc)` もこれに合わせてある。**2026-09-21 に実測で本家と一致していることを確認（13.1）。この決定は推測だったが結果的に正しかった** |
...
- `TODO(要確認)`: 標準4状態の `displayOrder` の実値（色は 12.1 / 13.6 で確定済み。
  実値は画面に出ないためAPIが要る）
- `TODO(要確認)`: 本家がゲストをどのAPIフィールドで判別しているか
- `TODO(要確認)`: プロジェクト作成時に本家が自動生成するマスタの内容
  （サンプルプロジェクトはテンプレート由来なので、空のプロジェクトを作らないと分からない）
- `TODO(要確認)`: `query` 検索が**コメント本文**まで見るか（詳細を含むことは 13.2 で確定）
- `TODO(要確認)`: 変更履歴に記録されるフィールドの網羅（状態は 13.5 で確認済み）
- `TODO(要確認)`: 課題が複数のマイルストーンを持つとき、ガントはどの終了日に帯を出すか
- `TODO(要確認)`: PR の `status.id` の 2 と 3 が何か（Gitリポジトリが無く実測できない）

2026-09-21 に解決したもの（上のリストから外した）:
`課題一覧の sort の既定値`（→ 13.1）、`keyword 検索が見る範囲`のうち詳細の扱い（→ 13.2）、
`ガントの四半期スケールの区切り方`（→ 13.4）、`標準4状態の色`（→ 13.6）。

2026-09-17 に解決済みだったが、このリストから消し忘れていたもの:
`添付ファイルのサイズ上限`（50MB）、`状態を「完了」にするとき完了理由が必須か`（→ 1.2 / 任意）、
`課題を削除したとき子課題がどうなるか`（子課題を持つ課題は削除できない）。
