...
## 1.1 状態の制約（2026-09-17 に一次情報で確認）

出典: https://backlog.com/ja/blog/backlog-update-2019-10-24/

> 既存の状態（未対応、処理中、処理済み、完了）の名前や色を変更することはできません。
> また、「未対応」と「完了」の並び順は最初と最後で固定されています。

- **標準4状態は名前も色も変更できない。** 追加した状態だけが編集できる
- 「未対応」は必ず先頭、「完了」は必ず末尾
- **追加できる状態は1プロジェクトにつき8つ**（標準4つと合わせて最大12）

## 1.2 完了理由（2026-09-17 に一次情報で確認）

出典: https://backlog.com/ja/enterprise-help/userguide/userguide26/

5種類。**必須という記述は無い**（任意と扱う）。

| 値 | 意味（原文） |
|---|---|
| 対応済み | 課題の対応をした場合に設定します |
| 対応しない | 必要がないと判断された課題に設定します |
| 無効 | 課題が間違えて登録された場合に設定します |
| 重複 | 同一課題が登録されていた場合に設定します |
| 再現しない | 課題に登録された内容がすでに再現しない場合に設定します |

状態を変えるときに、完了理由・担当者・マイルストーン・期限日・コメントを
同時に設定できる（状態を変えずに設定することもできる）。
